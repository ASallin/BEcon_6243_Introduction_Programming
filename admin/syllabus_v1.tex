...
\usepackage[utf8]{inputenc} % optional on newer LaTeX
...
\usepackage{bm}
%\usepackage{dsfont} % alternative to bbm
...
% --- Graphics & plots (only keep what you use) ---
\usepackage{graphicx}
...
\lstset{
  ...
  frame=single,                      % adds a thin frame
...
\lstdefinelanguage{R}{
  morekeywords={if, else, repeat, while, function, for, in, next, break, print},
  sensitive=true,
  morecomment=[l]{#},
  morestring=[d]"
}
...
\lstdefinestyle{Rstyle}{
  ...
  basicstyle=\ttfamily\small,
  keywordstyle=\color{purple}\bfseries, % color for keywords like "if", "function"
  stringstyle=\color{green!40!black},   % strings in green
  commentstyle=\color{gray},
  showstringspaces=false,
  frame=single,
...
\usepackage{dirtytalk}  % \say if you prefer
\usepackage{pdfpages}

% --- Bibliography (natbib + apalike.bst) ---
\usepackage{natbib}
...
\title{BA Course "Advanced Programming for Economists"}
\date{November, 2025} 
...
\begin{itemize}
    \item Elementary programming skills (see syllabus "Data Handling" course)
    \item Knowledge of basic Data structures (see syllabus "Data Handling" course)
    \item Statistics
\end{itemize}
...
\noindent The student assessment consists of two main components:
\begin{enumerate}
    \item \textbf{Final Exam (30\% of the grade)}: This exam will cover the core theoretical concepts of the course. Form: digital exam, BYOD, 60'. Multiple-choice questions. 
    \item \textbf{Group Project (70\% of the grade)}: This is the primary applied assessment. Students will work in groups of 4-5 to design and build a `software tool' that applies the skills learned in the course.\\
...
\begin{itemize}
    \item The course includes 10 tutorial sessions to accompany the lectures.
    \item A team of 4 Teaching Assistants (TAs) working on each exercise session.
    \item First 8 tutorials: (Skill Building)
    \begin{itemize}
        \item These are classic, hands-on sessions for students to practice the concepts from the lectures.
        \item They will be held as a single, combined session for all students. Or 
        \item 1 TA will lead the instruction, while the other 3 TAs provide hands-on support and answer individual questions.
    \end{itemize}
    \item Final 2 tutorials: (Project Support) 
    \begin{itemize}
        \item These sessions are structured as "office hours" for the group projects.
        \item Purpose: This time is dedicated for groups to meet with TAs to get help, troubleshoot problems, and receive direct guidance on their project.
    \end{itemize}
\end{itemize}
...
\toprule
\textbf{Lecture 1} & The big picture. Set up your environment. & Aurélien \& Franzi \\
\textbf{Lecture 2} & Working together: intro to version control and git & Aurélien \\
\textbf{Lecture 3} & Working together: more about version control and git & Aurélien \\
\textbf{Lecture 4} & Intro to python & Franzi \\
\textbf{Lecture 5} & Introduction to Classes (OOP) & Franzi \\
\textbf{Lecture 6} & Python for Data: Pandas & Franzi \\
\\
\textit{Break} &  &  \\
\\
\textbf{Lecture 7} & Error Handling, documentation, Debugging & Franzi \\
\textbf{Lecture 8} & Short overview of databases and relational database management systems & Aurélien \\
\textbf{Lecture 9} & Devops, Continuous Integration & Aurélien \\
\textbf{Lecture 10} & Guest Lecture & Guest\\
\textbf{Lecture 11} & Case Study: Building a Class for Geodata & Aurélien \& Franzi \\
\textbf{Lecture 12} & Conclusion & Aurélien \& Franzi \\
\\
\multicolumn{3}{l}{\textcolor{blue}{\textit{Optional alternatives (instead of 8 or 9)}}} \\
& \textcolor{blue}{Communication: dashboards / matplotlib} \\
& \textcolor{blue}{Machine Learning and basic predictions with PyTorch} &  \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection*{Lecture 1: The Big Picture. Set up your environment \textit{F, A}}
...
Students will install python, learn about tooling and package management with \href{https://docs.astral.sh/uv/}{UV} (as a replacement for pip), install and work with Visual Studio Code, and CLI (Command-Line Interface) to install everything. Students also learn about how to organize files and write file names correctly.

\textbf{Exercise week 1}: Students will have time to set up their environment and be ready for the lecture. They will also install git and create a github account.

\bigskip

\subsection*{Lecture 2-3: Working Together: Intro to Version Control and Git \textit{A}}
...
\textbf{Exercise weeks 2-3}: Students will have time to experiment with git and github.

\textbf{Sources}: 
\begin{itemize}
    \item \href{https://simson.io/intro-to-git/}{"Introduction to git"} from Jam Simson
    \item \href{https://rse-book.github.io/version-control.html}{"Git Version Control"}, "Research Software Engineering", Matt Bannert
\end{itemize}

\bigskip

\subsection*{Lecture 4: Intro to Python \textit{F}}
...
\textbf{Exercise week 4}: Students apply basic concepts in Python learned in the course.

\bigskip


\subsection*{Lecture 5: Introduction to Classes (OOP)} \textit{F}
...
\subsection*{Lecture 6: Python for Data  \textit{F}}
...
\textcolor{blue}{\textit{Optional: Time permitting, we may also provide a brief outlook on modern high-performance alternatives like Polars or tensor-based frameworks like PyTorch.}}

\textbf{Exercise week 6}: Students apply classes and work with data in Python.

\bigskip

\subsection*{Lecture 7: Error Handling, Documentation} \textit{F}
...
\subsection*{Lecture 8: Short overview of databases and relational database management systems (to drop if too much)} \textit{A}

This lecture introduces students to databases and relational database management systems. Using \href{https://www.postgresql.org/}{Postgresql}, students learn what are database systems in order to manage and process large data sets. They will be introduced to creating, manipulating, and querying databases using SQL.

\textbf{Exercise weeks 7-8}: Students run simple sql queries and set up a basic relational database management system.

\textbf{Sources}: 
\begin{itemize}
    \item "Big Data Analytics" from Ulrich Matter
    \item \href{https://rse-book.github.io/data-management.html#databases}{"Databases"}, "Research Software Engineering", Matt Bannert
\end{itemize}

\bigskip

\subsection*{Lecture 9: DevOps (to drop if too much)} \textit{A}
...
\textbf{Exercise weeks 9-10}: tbd.

\textbf{Sources}: 
\begin{itemize}
    \item \href{https://h4sci.github.io/blog/self-hosted-shiny-pg/}{Survey with a Database Backend}
    \item \href{https://github.com/h4sci/h4sci-gha-demo}{"Github Action Demo"}, Matt Bannert
\end{itemize}

\bigskip

\subsection*{Lecture 10: Guest Lecture} 
...
\subsection*{Lecture 11: Case Study: Building a Class for Geodata} \textit{F, A}
...
\subsection*{Lecture 12: Conclusion} \textit{F, A}
This lecture concludes the semester.

\textbf{Exercise week 11-12}: Q\&A for the Project.
...
\begin{itemize}
    ...
    \begin{itemize}
        \item These are classic, hands-on sessions for students to practice the concepts from the lectures.
        \item They will be held as a single session for all students, where one TA will lead the instruction while the other TA's will provide hands-on support an answer individual questions
    \end{itemize}
    \item Final 2 tutorials: (Project Support) 
    \begin{itemize}
        \item These sessions are structured as "office hours" for the group projects.
...
\textbf{TA Responsibilities \& Budget}
...
\begin{itemize}
    \item Grading of group projects and exams ($\sim$30 hours) \\ TAs will assist in evaluating the final group projects. This involves reviewing the code repositories for quality and style, verifying the functionality of the Python classes, and assessing the documentation notebooks (Analysis / User Guide).
    \item Content creation and preparation ($\sim$25 hours) \\ TAs will develop and refine the exercise materials and sample solutions for the 8 skill-building tutorials, ensuring they align with the weekly lecture content.
    \item Presence and teaching ($\sim$ 15 hours) \\ In addition to the lead teaching role (rotating), TAs provide intensive in-class support, which is critical for programming courses with beginners/intermediates.
    \item Student support and admin ($\sim$ 10 hours) \\ During the project phase, TAs act as technical mentors, helping groups structure their code and resolve Git/collaboration conflicts.
\end{itemize}
