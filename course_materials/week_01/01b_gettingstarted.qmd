...
| `touch hello.txt` | Create an empty text file named "hello" in `bash` |
| `echo. > hello.txt` | Create an empty text file named "hello" in `PowerShell` |
| `rm hello.txt` | Delete a file in `bash` (Careful! Can't be undone!) |
| `del hello.txt` | Delete a file in `PowerShell` (Careful! Can't be undone!) |
...
An isolated environment keeps each project's packages separate:

- **Without isolation:** If Project A needs `pandas` version 1.5 and Project B needs
  `pandas` version 2.0, they conflict! ❌

- **With isolation:** Each project has its own environment. Project A gets `pandas` 1.5,
  Project B gets `pandas` 2.0. No conflicts! ✅

This is different from R, where packages are typically installed globally.
:::
